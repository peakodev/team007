...
from agent_book import AgentBook, AgentBookIterator
from agent_notes import AgentNotes, format
...
def show_all():
    max_key_length = max(len(key) for key in book.keys())
    print(max_key_length)

    print("  | {} | {} | {} | {} | {} |".format('Agent call sign'.ljust(max_key_length), 'Email'.ljust(max_key_length),
                                                'Phones'.ljust(max_key_length), 'Birthday', 'Address'))
    for value in book.data.items():
        # print(value[1].call_sign)
        # print(value[1].email)
        Name = str(value[1].call_sign)
        email = str(value[1].email)
        phone = value[1].phones
        # birthday =
        # adress =
        # for phone in value[1].phones:
        #     print(phone)
        # if value[1].birthday:
        #     print(value[1].days_to_birthday())
        # if value[1].address:
        #     print(str(value[1].address))
        print("  | {:11} | {:11} | {:11} |   |   |".format(Name.ljust(max_key_length), email.ljust(max_key_length),
                                                           str(*phone)) + format.SEPARATOR)
